...
mkdir -p /var/backups/songmetrix
check_error "Falha ao criar diretório de backup"

# Backup do banco de dados usando PGPASSWORD
echo "Realizando backup do banco de dados..."
BACKUP_DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="/var/backups/songmetrix/music_log_${BACKUP_DATE}.sql"
PGPASSWORD=Conquista@@2 pg_dump -h localhost -U postgres music_log > "$BACKUP_FILE"
check_error "Falha ao criar backup do banco de dados"

echo "Backup do banco criado em: $BACKUP_FILE"
...
echo "2. Logs do PM2: pm2 logs songmetrix-api"
echo "3. Logs do Nginx: tail -f /var/log/nginx/songmetrix.access.log"
