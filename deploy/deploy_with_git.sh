...
if [ ! -d ".git" ]; then
    ...
    echo "Por favor, forneça a URL do repositório Git:"
    ...
    if [ "$(ls -A /var/www/songmetrix)" ]; then
        ...
        mv /var/www/songmetrix /var/www/songmetrix_backup_$(date +%Y%m%d_%H%M%S)
        mkdir -p /var/www/songmetrix
    ...
    # Clonar repositório
    git clone $GIT_URL .
    check_error "Falha ao clonar repositório do GitHub"
else
