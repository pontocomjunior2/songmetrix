...
export default function Layout({ currentView, onNavigate }: LayoutProps) {
  ...
  return (
    <div className={`min-h-screen flex ${theme === 'dark' ? 'dark' : ''}`}>
      ...
      {/* Mobile Sidebar */}
      <div className={`lg:hidden ${sidebarOpen ? 'block' : 'hidden'} w-full bg-gradient-to-b from-navy-600 via-navy-700 to-navy-800`}>
        <Sidebar currentView={currentView} onNavigate={handleNavigate} />
      </div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="space-y-2">
                        {notifications.map((notification) => (
                          <div
                            key={notification.id}
                            className={`p-3 rounded-lg ${notification.read ? 'bg-gray-50 dark:bg-gray-700' : 'bg-navy-50 dark:bg-navy-900/50'}`}
                          >
