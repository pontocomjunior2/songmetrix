...
import { Label } from "./label" // Corrigido - Assumindo que label.tsx existe aqui
...
const FormItem = React.forwardRef< HTMLDivElement, React.HTMLAttributes<HTMLDivElement> >(({ className, ...props }, ref) => {
  ...
  // Ajustar cn
  const resolvedCn = cn as (...inputs: any[]) => string;

  return (
    <FormItemContext.Provider value={{ id }}>
      <div ref={ref} className={resolvedCn("space-y-2", className)} {...props} />
...
const FormLabel = React.forwardRef< React.ElementRef<typeof LabelPrimitive.Root>, React.ComponentPropsWithoutRef<typeof LabelPrimitive.Root> >(({ className, ...props }, ref) => {
  ...
  return (
    <Label
      ref={ref}
      className={resolvedCn(error && "text-destructive", className)}
      htmlFor={formItemId}
      {...props}
    />
  )
...
const FormDescription = React.forwardRef< HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement> >(({ className, ...props }, ref) => {
  ...
  return (
    <p
      ...
      className={resolvedCn("text-sm text-muted-foreground", className)}
      {...props}
    />
  )
})
...
const FormMessage = React.forwardRef< HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement> >(({ className, children, ...props }, ref) => {
  ...
  return (
    <p
      ref={ref}
      id={formMessageId}
      className={resolvedCn("text-sm font-medium text-destructive", className)}
      {...props}
    >
      {body}
    </p>
