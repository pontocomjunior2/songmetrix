 ...
 */
export const checkAdminAuth = async (req: AuthenticatedRequest, res: Response, next: Function) => {
  try {
 ...
 */
adminRouter.post('/insights/generate', async (req: AuthenticatedRequest, res: Response) => {
  try {
    logger.info(`[AdminRoutes] Iniciando geração de insights solicitada por admin ${req.user?.id}`);

    // Instanciar serviços
    const llmService = new LlmService();
    const insightGenerator = new InsightGeneratorService(llmService);

    logger.info('[AdminRoutes] Serviços instanciados, iniciando processo em background');

    // Iniciar processo em background sem aguardar
    insightGenerator.generateInsightsForAllUsers()
      .then(async () => {
        logger.info('[AdminRoutes] Processo de geração de insights concluído com sucesso');
        await insightGenerator.close();
      })
      .catch(async (err) => {
        logger.error('[AdminRoutes] Falha no processo de geração de insights em background', {
          error: err.message,
          stack: err.stack,
          adminId: req.user?.id
        });
    ...
    logger.error('[AdminRoutes] Erro ao iniciar geração de insights', {
      error: error instanceof Error ? error.message : 'Erro desconhecido',
      stack: error instanceof Error ? error.stack : undefined,
      adminId: req.user?.id
    });
 ...
 */
adminRouter.get('/insights/drafts', async (req: AuthenticatedRequest, res: Response) => {
  try {
    logger.info(`[AdminRoutes] Buscando rascunhos de insights solicitado por admin ${req.user?.id}`);
    ...
    if (draftsError) {
      logger.error('[AdminRoutes] Erro ao buscar rascunhos', {
    ...
    logger.info(`[AdminRoutes] Encontrados ${drafts?.length || 0} rascunhos`, {
    ...
    logger.error('[AdminRoutes] Erro inesperado ao buscar rascunhos', {
      error: error instanceof Error ? error.message : 'Erro desconhecido',
      stack: error instanceof Error ? error.stack : undefined,
      adminId: req.user?.id
    });
 ...
 */
adminRouter.post('/insights/:id/approve', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;

    logger.info(`[AdminRoutes] Aprovando insight ${id} por admin ${req.user?.id}`);
    ...
    if (updateError) {
      logger.error(`[AdminRoutes] Erro ao aprovar insight ${id}`, {
    ...
    if (!updatedEmail) {
      logger.warn(`[AdminRoutes] Insight ${id} não encontrado para aprovação`);
    ...
    logger.info(`[AdminRoutes] Insight ${id} aprovado com sucesso por admin ${req.user?.id}`);
    ...
    logger.error(`[AdminRoutes] Erro inesperado ao aprovar insight ${req.params.id}`, {
      error: error instanceof Error ? error.message : 'Erro desconhecido',
      stack: error instanceof Error ? error.stack : undefined,
      adminId: req.user?.id
    });
 ...
 */
adminRouter.post('/insights/:id/send', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;

    logger.info(`[AdminRoutes] Enviando insight ${id} solicitado por admin ${req.user?.id}`);
    ...
    if (fetchError || !emailData) {
      logger.warn(`[AdminRoutes] Insight ${id} não encontrado para envio`, {
    ...
    if (emailData.status !== 'approved') {
      logger.warn(`[AdminRoutes] Tentativa de envio de insight ${id} com status ${emailData.status}`, {
    ...
    try {
      ...
      logger.info(`[AdminRoutes] Enviando e-mail para ${emailData.users?.email}`, {
      ...
      if (emailResult.success) {
        ...
        if (sentError) {
          logger.error(`[AdminRoutes] Erro ao atualizar status após envio do insight ${id}`, {
        ...
        logger.info(`[AdminRoutes] Insight ${id} enviado com sucesso para ${emailData.users?.email}`, {
          recipient: emailData.users?.email,
          adminId: req.user?.id
        });
        ...
        // Falha no envio
        logger.error(`[AdminRoutes] Falha no envio do insight ${id}`, {
      ...
      logger.error(`[AdminRoutes] Erro no serviço de e-mail para insight ${id}`, {
        error: emailServiceError instanceof Error ? emailServiceError.message : 'Erro desconhecido',
        stack: emailServiceError instanceof Error ? emailServiceError.stack : undefined,
        adminId: req.user?.id
      });
    ...
    logger.error(`[AdminRoutes] Erro inesperado ao enviar insight ${req.params.id}`, {
      error: error instanceof Error ? error.message : 'Erro desconhecido',
      stack: error instanceof Error ? error.stack : undefined,
      adminId: req.user?.id
    });
