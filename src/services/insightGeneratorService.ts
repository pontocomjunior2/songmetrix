...
import { musicLogDbPool } from '../config/musicLogDb';
...
interface GrowthTrendInsight {
  artist: string;
  song_title: string;
  current_week_plays: number;
  previous_week_plays: number;
}

/**
 * Serviço para geração de insights musicais para usuários
 * Analisa dados de execução musical e gera conteúdo personalizado via LLM
 */
export class InsightGeneratorService {
   ...
   * @param llmService Instância do serviço LLM para geração de conteúdo
   */
   ...
   */
  public async generateInsightsForAllUsers(): Promise<void> {
    try {
   ...
   * Utiliza a conexão music_log para consultas analíticas
   * @param user Objeto com id e email do usuário
   * @returns Dados do insight ou null se não encontrado
   */
  private async _findGrowthTrendInsight(user: { id: string, email: string }): Promise<GrowthTrendInsight | null> {
